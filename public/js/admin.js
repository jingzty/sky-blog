/*
 * 后台共享脚本：侧栏注入、lucide 图标、退出登录、toast
 * 依赖：lucide.js、api.js（页面已引入）
 */
(function () {
  const NAV = [
  ...
  function toast(msg) {
    const old = document.querySelector('.pi-toast');
    if (old) old.remove();
  ...
  document.addEventListener('DOMContentLoaded', () => {
    // 注入侧栏到 <body data-sidebar> 占位，或 body 开头
  ...
  // 暴露 toast（供页面脚本使用，亦可使用 ui.js 的 UI.toast）
  window.Admin = { toast };
})();
